...
/** @file hpc_blastn_app.cpp
 * HPC-BLASTN command line application
 */
...
#include "hpc_blast_agglomerate.h"
...
int CBlastnApp::Run(void)
{
    ...
    char **buffer = 0;
    char *gbl_buffer = 0;   // For combined queries.
    int *bsize = 0;
...
my_oid_stop =  ( local_tid + 1 )*(num_seqs_per_team_leader/10);
	...
	//ceb
	  ...
	  lcl_query_input_stream.reset(new CNcbiIstream(&lcl_query_membuf));
          ...
          CBlastInput lcl_input(&lcl_fasta);
#else
...
//#endif
      ...
      if (batch_size) {
#if UPDATE
	lcl_input.SetBatchSize(batch_size);
#else
	...
	if ( tid == 0 )
	  total_len = formatter->GetDbTotalLength();
#pragma omp flush(total_len)
#pragma omp barrier
...
#if UPDATE
	lcl_input.SetBatchSize(mixer.GetBatchSize());
#else
	...
	int batch_count=0;
	  ...
	  if ( got_work )  
...
#if TIMING
               //t_timer[9] = omp_get_wtime();
               double t_presearch = omp_get_wtime();
#endif //TIMING

#if UPDATE
	    lcl_blast->Run();
	    if (!batch_size)
              lcl_input.SetBatchSize(mixer.GetBatchSize(lcl_blast->GetNumExtensions()));
#else      
	  ...
	  else
	    {
	      ...
	      //tQueryInfo[tid] =     BlastQueryInfoNew(eBlastTypeBlastn,0);//{0,0,0,NULL,0,NULL};
...
//#endif
    ...
    if ( lintpBatchEndPosition )  {
      free(lintpBatchEndPosition);
      lintpBatchEndPosition=NULL;
    }

    if ( lintpBatchStartPosition )  {
      free(lintpBatchStartPosition);
      lintpBatchStartPosition=NULL;
    }

    for ( int i=0; i< num_thread_groups; ++i )  {
      if ( lintppThreadGroupQueryBufferEndPosition[i] )  {
	free( lintppThreadGroupQueryBufferEndPosition[i] );
	lintppThreadGroupQueryBufferEndPosition[i]=NULL;
      }
    }
    if ( lintppThreadGroupQueryBufferEndPosition )  {
      free( lintppThreadGroupQueryBufferEndPosition );
      lintppThreadGroupQueryBufferEndPosition=NULL;
    }
...
BlastQueryInfo *gbqipfncCombineThreadGroupBlastQueryInfo(int aintNumberOfThreadGroups, BlastQueryInfo **abqippBlastQueryInfo) 
{
  ...
  if(lintTotalNumberOfQueries==0){ return  BlastQueryInfoNew(eBlastTypeBlastn,0);}

  int lintCount = 0;

  BlastQueryInfo *lbqipBlastQueryInfo = BlastQueryInfoNew(eBlastTypeBlastn,lintTotalNumberOfQueries);
